<#
.SYNOPSIS
    WinOpt - Optimizador de Windows (versiÃ³n compilada)
.NOTES
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <StackPanel>
        <GroupBox Header="Tweaks" Margin="5">
            <StackPanel>   <!-- Este StackPanel envuelve los checkboxes, resolviendo el error del GroupBox -->
        ...
        <GroupBox Header="Opciones" Margin="5">
            <CheckBox Name="chkAnalyzeOnly" Content="Modo solo anÃ¡lisis" Margin="3"/>
        </GroupBox>
        <Button Name="btnRunTweaks" Content="Ejecutar tweaks seleccionados" Margin="5"/>
        <GroupBox Header="InstalaciÃ³n de aplicaciones" Margin="5">
            <StackPanel>
...
#region core.ps1
# core.ps1 â€“ Tabla sincronizada, logging, runspaces
...
#region tweaks.ps1
# tweaks.ps1 â€“ Backup, restore, test y aplicaciÃ³n de tweaks
...
function Invoke-TweakAction {
    ...
    if ($global:sync.chkAnalyzeOnly.IsChecked) {
        & $LogFunc "[ANALYZE] Se aplicarÃ­a: $TweakId - $($TweakDef.content)"
        return $true
    }
...
#region fixes.ps1
# fixes.ps1 â€“ Reparaciones del sistema
function Repair-SFC {
    if ($global:sync.chkAnalyzeOnly.IsChecked) { & $global:sync.WriteLog "[ANALYZE] Se ejecutarÃ­a sfc /scannow"; return }
    & $global:sync.WriteLog "Ejecutando SFC /SCANNOW..."
    try { $output = sfc /scannow 2>&1; $output | ForEach-Object { & $global:sync.WriteLog $_ } } catch { & $global:sync.WriteLog "âŒ Error en SFC: $_" }
}
function Repair-DISM {
    if ($global:sync.chkAnalyzeOnly.IsChecked) { & $global:sync.WriteLog "[ANALYZE] Se ejecutarÃ­a DISM /RestoreHealth"; return }
    & $global:sync.WriteLog "Ejecutando DISM /RestoreHealth..."
    try { $output = dism /online /cleanup-image /restorehealth 2>&1; $output | ForEach-Object { & $global:sync.WriteLog $_ } } catch { & $global:sync.WriteLog "âŒ Error en DISM: $_" }
}
function Repair-WindowsUpdate {
    if ($global:sync.chkAnalyzeOnly.IsChecked) { & $global:sync.WriteLog "[ANALYZE] Se repararÃ­a Windows Update"; return }
    ...
    try {
        ...
        & $global:sync.WriteLog "âœ… Windows Update reparado."
    } catch { & $global:sync.WriteLog "âŒ Error reparando Windows Update: $_" }
}
function Repair-Network {
    if ($global:sync.chkAnalyzeOnly.IsChecked) { & $global:sync.WriteLog "[ANALYZE] Se repararÃ­a la red"; return }
    ...
    try {
        ...
        ipconfig /renew
        ipconfig /flushdns
        & $global:sync.WriteLog "âœ… Red reparada. Se recomienda reiniciar."
    } catch { & $global:sync.WriteLog "âŒ Error reparando red: $_" }
}
function Repair-All {
...
#region install.ps1
# install.ps1 â€“ InstalaciÃ³n de aplicaciones
...
function Install-Package {
    ...
    if ($global:sync.chkAnalyzeOnly.IsChecked) { & $LogFunc "[ANALYZE] InstalarÃ­a: $AppId"; return $true }
    if ($wingetId -and (Test-WingetAvailable)) {
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) { & $LogFunc "âœ… InstalaciÃ³n exitosa: $AppId"; return $true }
        } catch { & $LogFunc "âŒ Winget fallÃ³ para ${AppId}: $_" }
    }
    if ($chocoId -and (Test-ChocoAvailable)) {
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) { & $LogFunc "âœ… InstalaciÃ³n exitosa mediante Chocolatey: $AppId"; return $true }
        } catch { & $LogFunc "âŒ Chocolatey fallÃ³ para ${AppId}: $_" }
    }
    & $LogFunc "âŒ No se pudo instalar $AppId"
    return $false
}
...
function Install-MultipleApps {
    ...
    if (-not (Test-Path $appsFile)) { & $LogFunc "No se encontrÃ³ applications.json"; return }
    ...
    foreach ($app in $AppNames) {
        ...
        else { & $LogFunc "AplicaciÃ³n '$app' no definida en applications.json" }
    }
}
...
#region cli.ps1
# cli.ps1 â€“ Modo lÃ­nea de comandos (implementaciÃ³n completa)
if ($Tweaks -or $InstallApps -or $AnalyzeOnly -or $RestoreFromBackup -or $Silent -or $NoRestart -or $Force) {
    ...
    Write-Log "ParÃ¡metros: Tweaks='$Tweaks', InstallApps='$InstallApps', AnalyzeOnly=$AnalyzeOnly"
    ...
    if (-not $NoRestart -and -not $AnalyzeOnly) {
        $restartChoice = if ($Silent) { "Y" } else { Read-Host "Â¿Reiniciar ahora? (Y/N)" }
...
#region gui.ps1
# gui.ps1 â€“ LÃ³gica de la interfaz grÃ¡fica
...
function Show-WinOptGUI {
    ...
    $global:sync.btnInstallApps.Add_Click({
        ...
        if ($selected.Count -eq 0) { & $global:sync.WriteLog "No se seleccionÃ³ ninguna aplicaciÃ³n."; return }
